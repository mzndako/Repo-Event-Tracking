 ...
 */
const getAllActors = () => {
  let sql = 'SELECT actors.*, count(events.id) as event_count, max(created_at) as date from actors inner join events on actors.id = events.actor_id GROUP BY events.actor_id order by event_count DESC'
